...
export class ThanhToan extends Component {
  tienGhe = (dsghe) => {
    if (dsghe.length < 1) return 0;

    return dsghe.reduce(
      (acumorlator, currentValue) => acumorlator + currentValue.giaVe,
      0
    );
  };
  tienCombo = (dsCombo) => {
    if (dsCombo.length < 1) return 0;

    return dsCombo.reduce(
      (acumorlator, currentValue) =>
        acumorlator + currentValue.price * currentValue.quanty,
      0
    );
  };
  datVeXemPhim = () => {
    ...
    if (danhSachGheChon.length > 0) {
      const danhSachVe = danhSachGheChon.map((item) => ({
        maGhe: item.maGhe,
        giaVe: item.giaVe,
      }));
      let data = {
      ...
      callAPI(
        {
          ...
          headers: {
            Authorization:
              "Bearer " +
              JSON.parse(localStorage.getItem(nameOfLocalStore.TaiKhoan))
                .accessToken,
          },
        },
        (data) => {
    ...
    callAPI(
      {
        ...
        data: {
          taiKhoan: JSON.parse(localStorage.getItem(nameOfLocalStore.TaiKhoan))
            .taiKhoan,
        },
  ...
  render() {
    ...
    const active =
      danhSachGheChon.length > 0
        ? "thanhtoan_bottom active"
        : "thanhtoan_bottom";
        ...
        <div className="thanhtoan">
          <div className="thanhtoan_top">
            <div className="thanhtoan_tongtien">
              <h5>
                {danhSachGheChon && dsComBo
                  ? `${
                      this.tienGhe(danhSachGheChon) + this.tienCombo(dsComBo)
                    } đ`
                  : "0 đ"}
              </h5>
            </div>
            ...
            <div className="content_thanhtoan vitrighe">
              <div className="item1 ghechon">
                <p>Ghế</p>
                {danhSachGheChon.map((item, index) => (
                  <p key={index}>{item.tenGhe}</p>
                ))}
              </div>
              <div className="item2 tien-ghe">
                <span>
                  {danhSachGheChon && danhSachGheChon.length > 0
                    ? `${this.tienGhe(danhSachGheChon)} đ`
                    : "0 đ"}
                </span>
              </div>
            </div>
            <div className="content_thanhtoan combo">
              <div className="item1 choncombo" onClick={this.diplayCombo}>
                <img
                  src="https://mcinox.cgv.vn/cinox/atomicDownload/downloadFile.fo?ATTCH_NO=203826&SEQ_NO=304112"
                  alt="abc"
                />
                <p>Chọn combo</p>
              </div>
              <div className="item2 tiencombo">
                <span>
                  {dsComBo && dsComBo.length > 0
                    ? `${this.tienCombo(dsComBo)} đ`
                    : "0 đ"}
                </span>
              </div>
            </div>
